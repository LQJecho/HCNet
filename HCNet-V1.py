...
path = './model/HCNet-V1.h5'
checkpoint = ModelCheckpoint(path, monitor='val_loss', verbose=1, save_best_only=True,
                             mode='min', save_weights_only=True)
model = channel_estimation()
model.fit([train_y,train_pnr], train_label, batch_size=2048, epochs=100, verbose=2,
          callbacks=[checkpoint], validation_data=([val_y,val_pnr], val_label), shuffle=True)
...
for pnr_db in PNR_db:
    ...
    mse=model.evaluate([test_y,test_pnr],test_label,batch_size=test_size)
    MSE.append(mse)
