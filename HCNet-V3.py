...
def channel_estimation():
    rate=0.1
    y = Input(shape=(30,),dtype=tf.float32)
    index=Input(shape=(15,),dtype=tf.float32)
...
train_y=train_y[:,0:30]
train_index=train_index[:,0:15]
val_y=val_y[:,0:30]
val_index=val_index[:,0:15]
path = './model/HCNet-V3.h5'
