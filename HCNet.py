...
# Fig4, curve HCNet
PNR_db=np.arange(-20,10,5)
RF=np.arange(8,10,2)
for rf in RF:
    MSE = []
    for pnr_db in PNR_db:
        dir = '../dataset/RF/{rf}RF/pnr{pnr}.mat'
        dir=dir.format(rf=rf,pnr=pnr_db)
        ...
        mse=model.evaluate([test_y,test_index,test_pnr],test_label,batch_size=test_size,verbose=0)
        doa=model.predict([test_y,test_index,test_pnr],batch_size=test_size)
        MSE.append(mse)
    print('MSE',rf,'_NN=',MSE)

# Fig5, curve HCNet, snr-10dB
RF=np.arange(3,8,1)
MSE = []
for rf in RF:
    dir = '../dataset/RF/pnr-10/{rf}RF.mat'
    dir = dir.format(rf=rf)
    test_index = sio.loadmat(dir)['Index']
    test_y_c = sio.loadmat(dir)['Y']
    test_y = np.real(test_y_c * np.conj(test_y_c))
    pnr = np.power(10, -10 / 10)
    test_pnr = pnr * np.ones([test_size, 1])
    test_y = test_y[0:test_size, :]
    test_index = test_index[0:test_size, :]
    mse = model.evaluate([test_y, test_index, test_pnr], test_label, batch_size=test_size,verbose=0)
...
# Fig6, curve6, q_tr=inf
Qbit=np.arange(3,8,1)
MSE = []
for Q in Qbit:
    dir = '../dataset/RF/pnr0/{Q}RF.mat'
    dir = dir.format(Q=Q)
    test_index = sio.loadmat(dir)['Index']
    test_y_c = sio.loadmat(dir)['Y']
    test_y = np.real(test_y_c * np.conj(test_y_c))
